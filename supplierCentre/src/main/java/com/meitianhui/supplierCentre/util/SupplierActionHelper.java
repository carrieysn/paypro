...
public class SupplierActionHelper {


	/***
	 * 商家的token前缀
	 */
	private final static String TOKEN_PREFIX = "merchant_token_";
	
	/***
	 * token客户端cookie标识
	 */
	private final static String TOKEN_COOKIE_NAME = "MERCHANTTOKEN";
	/** session过期时长 120分钟**/
	private final static int SESSION_EXPIRED_TIME = 120 * 60;
	 ...
	 * @date   2017年4月13日
	 */
	public static void setSessionUser(HttpServletResponse  response, SessionSupplier user){
	 ...
	 * @date   2017年4月13日
	 */
	public static SessionSupplier getSessionUser(HttpServletRequest request){
		Cookie[] cookies = request.getCookies();
		if(cookies != null && cookies.length > 0){
			for(Cookie cookie : cookies){
				if(TOKEN_COOKIE_NAME.equals(cookie.getName())){
					String key = TOKEN_PREFIX + cookie.getValue();
					SessionSupplier user = (SessionSupplier) RedisUtils.get(key);
					if(user != null){
