	...
	public @ResponseBody ResultVO<Object> accountLogin(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			//if(validCode.equalsIgnoreCase(RedisUtils.get(code_cache_prefix + mobile) + "")){	//验证通过
				...
				Map<String, Object> supplierBusiness = preSupplierService.querySupplierBusinessInfo(user.getId());
				user.setBusiness_type_key(supplierBusiness.get("business_type_key") + "");
				user.setBusiness_type((supplierBusiness.get("business_type_val") + "").replace(",", " "));
				...
				if(null != userInfo){
					...
					user.setServiceMobile(PlatformApiUtil.formatStr(userInfo.get("mobile")));
				}
